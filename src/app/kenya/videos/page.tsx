...
const MarketIndicator = ({ data }: { data: MarketData }) => (
  <div className="flex flex-col space-y-2 p-6 bg-[#F9F8F5] border-l-2 border-[#8B7355]">
    <div className="flex items-center justify-between">
      <span className="text-[10px] uppercase tracking-[0.2em] text-[#2C2C2C]/60 font-semibold">
        {data.indicator}
      </span>
      {data.trend === 'up' ? (
        <TrendingUp size={14} className="text-[#2C2C2C]" />
      ) : data.trend === 'down' ? (
        <TrendingUp size={14} className="text-[#8B7355] rotate-180" />
      ) : (
        <div className="w-2 h-2 rounded-full bg-[#8B7355]" />
      )}
    </div>
    <div className="flex items-baseline space-x-3">
      <span className="text-2xl font-serif text-[#2C2C2C]">{data.q2025}</span>
...
export default function VideosPage() {
    ...
    "@graph": [
      {
        ...
        "publisher": {
          "@type": "Organization",
          "name": "Murivest",
          "logo": { "@type": "ImageObject", "url": "https://www. murivest.com/logo.webp" }
        },
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F5] text-[#2D3436] pt-24 pb-20 font-sans selection:bg-[#8B7355]/30">
      <SchemaInjector data={structuredData} />

      {/* --- Editorial Header --- */}
      <header className="container mx-auto px-6 mb-16 max-w-6xl">
        <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-[#2C2C2C]/10 pb-8">
          <div className="space-y-4 max-w-2xl">
            ...
            <h1 className="text-4xl md:text-6xl font-serif text-[#2C2C2C] leading-[1.1] tracking-tight">
              Commercial Real Estate <em className="italic text-[#8B7355]">Intelligence</em>
            </h1>
          ...
          <div className="mt-8 md:mt-0 text-right hidden md:block">
            ...
            <div className="font-serif text-xl text-[#2C2C2C]">Q1 2026 Outlook</div>
            <div className="text-xs text-[#8B7355] mt-1">Vol. IV • March 2026</div>
          </div>
        </div>
      </header>
      ...
      <main className="container mx-auto px-6 max-w-6xl">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-2/3 space-y-12">
            ...
            {/* Video Player */}
            <div className="relative aspect-video w-full bg-[#2C2C2C] shadow-2xl overflow-hidden rounded-sm">
              <iframe
            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center gap-4 text-[10px] uppercase tracking-[0.2em] font-semibold text-[#2D3436]/60">
                ...
                <span className="px-2 py-1 bg-[#2C2C2C]/5 text-[#2C2C2C] rounded-full">
              ...
              <div className="flex items-center gap-4 py-4 border-y border-[#2C2C2C]/10">
                <div className="h-12 w-12 rounded-full bg-[#2C2C2C] flex items-center justify-center text-[#F9F8F5] font-serif text-lg">
                  {activeVideo.expert.split(' ').map(n => n[0]).join('')}
                </div>
                <div>
                  <div className="font-serif text-lg text-[#2C2C2C]">{activeVideo.expert}</div>
                ...
                <button 
                  ...
                  className={`ml-auto px-6 py-2 text-[10px] font-bold uppercase tracking-widest transition-all border ${
                    isSubscribed 
                      ? 'bg-[#2C2C2C] text-[#F9F8F5] border-[#2C2C2C]' 
                      : 'bg-transparent text-[#2C2C2C] border-[#2C2C2C] hover:bg-[#2C2C2C] hover:text-[#F9F8F5]'
                  }`}
                >
            ...
            <article className="prose prose-lg max-w-none">
              <div className="flex items-center gap-3 mb-6 text-[#8B7355]">
                <FileText size={18} />
                <span className="text-[10px] uppercase tracking-[0.3em] font-bold">Executive Abstract</span>
              </div>
              
              <div className="text-[#2D3436]/80 leading-loose text-lg font-light space-y-6 border-l-2 border-[#8B7355] pl-6">
                <p>{activeVideo.abstract}</p>
                
                <div className="bg-[#2C2C2C]/[0.02] p-6 mt-6">
                  <h3 className="text-[#2C2C2C] font-serif text-lg mb-4 flex items-center gap-2">
                    <BarChart3 size={16} /> Key Metrics
                  </h3>
                  <div className="grid grid-cols-2 gap-4">
                    {activeVideo.keyData?.map((data, idx) => (
                      <div key={idx} className="space-y-1">
                        <div className="text-2xl font-serif text-[#2C2C2C]">{data.value}</div>
                        <div className="text-[10px] uppercase tracking-wider text-[#2D3436]/60">{data.metric}</div>
                        <div className="text-[9px] text-[#8B7355]">via {data.source}</div>
                      </div>
                    ))}
                  </div>
                </div>

                <h3 className="text-[#2C2C2C] font-serif text-xl mt-8 mb-4">Strategic Implications</h3>
                <p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {VIDEO_LIBRARY.filter(v => v.id !== activeVideo.id).map((video) => (
                  <button 
                    ...
                    className="group text-left space-y-3 p-4 hover:bg-white transition-colors rounded-sm border border-transparent hover:border-[#2C2C2C]/5"
                  >
                    <div className="aspect-video bg-[#2C2C2C]/5 relative overflow-hidden rounded-sm">
                      <img 
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="h-10 w-10 rounded-full bg-[#F9F8F5]/90 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                          <Play size={16} className="text-[#2C2C2C] fill-current ml-1" />
                        </div>
                      </div>
                    </div>
                    <div>
                      <div className="flex items-center gap-2 text-[9px] uppercase tracking-widest text-[#8B7355] mb-2">
                        {video.category} • {video.duration}
                      </div>
                      <h4 className="font-serif text-lg text-[#2C2C2C] leading-snug group-hover:text-[#8B7355] transition-colors">
                        {video.title}
                      </h4>
                    </div>
                  </button>
          ...
          <aside className="lg:w-1/3 space-y-8">
            <div className="sticky top-24">
              ...
              {/* Data Room CTA */}
              <div className="bg-[#2C2C2C] text-[#F9F8F5] p-8 shadow-xl mb-8">
                <ShieldCheck size={32} className="text-[#8B7355] mb-4" />
                ...
                <Link 
                  href="/contact" 
                  className="inline-flex items-center gap-2 text-[10px] uppercase tracking-widest font-bold text-[#8B7355] hover:text-[#F9F8F5] transition-colors"
                >
              ...
              {/* Featured Mandate */}
              <div className="border border-[#2C2C2C]/10 bg-white p-6 mb-8">
                <div className="text-[9px] uppercase tracking-[0.3em] text-[#8B7355] font-bold mb-3">Featured Mandate</div>
                <h4 className="font-serif text-xl text-[#2C2C2C] mb-3">Best Western Meridian Hotel</h4>
                <div className="space-y-2 text-sm text-[#2D3436]/70 mb-4">
                  <div className="flex justify-between">
                    <span>Target IRR</span>
                    <span className="font-serif text-[#2C2C2C]">15.2%</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <Link 
                  ...
                  className="block w-full py-3 text-center text-[10px] uppercase tracking-widest font-bold border border-[#2C2C2C] text-[#2C2C2C] hover:bg-[#2C2C2C] hover:text-[#F9F8F5] transition-all"
                >
                  View Offering
                </Link>
              </div>
              ...
              {/* Research Sources */}
              <div className="p-6 border-t border-[#2C2C2C]/10">
                ...
                <ul className="space-y-3 text-xs text-[#2D3436]/60">
                  <li className="flex items-start gap-2">
                    <Bookmark size={12} className="mt-0.5 text-[#8B7355]" />
              ...
              <div className="p-6 text-[10px] leading-relaxed text-[#2D3436]/40 italic border-t border-[#2C2C2C]/5">
