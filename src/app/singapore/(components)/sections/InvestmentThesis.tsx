    ...
    <section className="py-20 md:py-32 bg-[#1F1E1C] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16">
        <SectionHeader
          ...
          subtitle="Three structural pillars underpinning Singapore as a defensible commercial real estate allocation in Asia."
          align="center"
          dark
          className="mb-16"
        />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {INVESTMENT_THESIS.map((pillar, i) => (
            <ScrollReveal key={pillar.title} delay={i * 0.15}>
              <div className="border border-white/10 p-8 md:p-10 hover:border-[#8B7355]/40 transition-all duration-500 group h-full">
                {/* Icon */}
                ...
                <h3 className="font-serif text-2xl text-[#F8F7F4] mb-2 leading-tight">
                  {pillar.title}
                </h3>
                <p className="text-[11px] tracking-[0.15em] uppercase text-[#8B7355] mb-6">
                  {pillar.subtitle}
                </p>
                ...
                <div className="space-y-3 pt-6 border-t border-white/10">
                  {pillar.stats.map((stat) => (
                    <div key={stat.label} className="flex items-center justify-between">
                      ...
                      <span className="font-mono text-sm text-[#8B7355]">{stat.value}</span>
                    </div>
                  ))}
                </div>
              </div>
