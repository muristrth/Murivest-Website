      ...
      <header className="bg-white border-b border-[#E5E2DC] sticky top-0 z-30">
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[11px] tracking-wider uppercase text-[#5A5A5A]">Investor Portal</span>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-[11px] text-[#5A5A5A]">Welcome, <span className="text-[#2C2C2C]">Investor</span></span>
            <Link href="/singapore" className="text-[10px] tracking-wider uppercase text-[#8B7355] hover:text-[#2C2C2C] transition-colors">
              Exit Portal
            </Link>
          </div>
        </div>
                ...
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-3 text-[12px] tracking-wider uppercase transition-colors ${
                    item.active
                      ...
                      : 'text-[#5A5A5A] hover:bg-white hover:border-[#E5E2DC]'
                  }`}
                >
                  {item.icon}
                  {item.label}
                ...
                <ScrollReveal key={stat.label}>
                  <div className="bg-white border border-[#E5E2DC] p-6">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-[9px] tracking-[0.2em] uppercase text-[#5A5A5A]">{stat.label}</span>
                      <div className="text-[#8B7355]">{stat.icon}</div>
                    </div>
                    <p className="font-mono text-2xl text-[#2C2C2C] mb-1">{stat.value}</p>
                    <p className="text-[10px] text-[#5A5A5A]">{stat.change}</p>
                  </div>
                </ScrollReveal>
              ))}
            </div>
            ...
            <ScrollReveal>
              <div className="bg-white border border-[#E5E2DC]">
                <div className="p-6 border-b border-[#E5E2DC] flex items-center justify-between">
                  ...
                  <Link href="/singapore/investor-portal/deals" className="text-[10px] tracking-wider uppercase text-[#8B7355] hover:text-[#2C2C2C] transition-colors">
                    View All
                  </Link>
                </div>
                <div className="divide-y divide-[#E5E2DC]">
                  {SAMPLE_PROPERTIES.slice(0, 3).map((property) => (
                    <div key={property.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-[#F8F7F4] transition-colors">
                      <div className="flex-1">
                        <h3 className="font-serif text-base text-[#2C2C2C] mb-1">{property.title}</h3>
                        <p className="text-[11px] text-[#5A5A5A]">{property.address}</p>
                      </div>
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          <p className="text-[9px] tracking-wider uppercase text-[#5A5A5A]">Price</p>
                          <p className="font-mono text-sm text-[#2C2C2C]">{property.price}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[9px] tracking-wider uppercase text-[#5A5A5A]">Yield</p>
                          <p className="font-mono text-sm text-[#8B7355]">{property.yield}%</p>
                        </div>
                        <Link
                          href={`/singapore/properties/${property.slug}`}
                          className="flex items-center gap-1 text-[10px] tracking-wider uppercase text-[#8B7355] hover:text-[#2C2C2C] transition-colors"
                        >
            ...
            <ScrollReveal>
              <div className="bg-white border border-[#E5E2DC]">
                <div className="p-6 border-b border-[#E5E2DC] flex items-center justify-between">
                  ...
                  <Link href="/singapore/investor-portal/documents" className="text-[10px] tracking-wider uppercase text-[#8B7355] hover:text-[#2C2C2C] transition-colors">
                    View All
                  </Link>
                </div>
                <div className="divide-y divide-[#E5E2DC]">
                  {[
                    ...
                    <div key={doc.name} className="p-6 flex items-center justify-between hover:bg-[#F8F7F4] transition-colors">
                      <div className="flex items-center gap-3">
                        <FileText className="w-4 h-4 text-[#5A5A5A]" strokeWidth={1.5} />
                        <div>
                          <p className="text-sm text-[#2C2C2C]">{doc.name}</p>
                          <p className="text-[10px] text-[#5A5A5A]">{doc.type} • {doc.size}</p>
                        </div>
                      </div>
                      <button className="flex items-center gap-1 text-[10px] tracking-wider uppercase text-[#8B7355] hover:text-[#2C2C2C] transition-colors">
