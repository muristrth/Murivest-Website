            ...
            <AnimatePresence mode="wait">
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
                  ...
                  <InfoBox>The LR Number (Land Reference) is mandatory and will be verified against land registry records. Submissions without a valid LR Number will be auto-discarded.</InfoBox>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <InputField label="Property Name" value={form.propertyName} onChange={v => setField('propertyName', v)} required placeholder="e.g. Westpark Towers, Absa Center" hint="Official or commonly known name" />
                    <InputField label="LR Number (Land Reference)" value={form.lrNumber} onChange={v => setField('lrNumber', v)} required placeholder="e.g. 209/10842/1" hint="As per Certificate of Title. Mandatory." />
                    <InputField label="Title Deed Number" value={form.titleDeedNumber} onChange={v => setField('titleDeedNumber', v)} placeholder="e.g. IR 45678" />
                    <InputField label="Plot Number" value={form.plotNumber} onChange={v => setField('plotNumber', v)} placeholder="e.g. L.R. No. 209/10842/1" />
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                    <SelectField label="County Registered" value={form.countyRegistered} onChange={v => setField('countyRegistered', v)} options={KENYAN_COUNTIES.map(c => ({ value: c, label: c }))} required />
                    <InputField label="City / Town" value={form.locationCity} onChange={v => setField('locationCity', v)} required placeholder="e.g. Nairobi" />
                    <InputField label="Neighborhood / Estate" value={form.locationNeighborhood} onChange={v => setField('locationNeighborhood', v)} placeholder="e.g. Westlands, Upper Hill" />
                  </div>

                  <InputField label="Submarket" value={form.locationSubmarket} onChange={v => setField('locationSubmarket', v)} placeholder="e.g. Westlands CBD, Karen-Langata" hint="Specific submarket within the city" />
                  <InputField label="Full Street Address" value={form.streetAddress} onChange={v => setField('streetAddress', v)} placeholder="e.g. Waiyaki Way, off Muthangari Road" />

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <SelectField label="Title Type" value={form.titleType} onChange={v => setField('titleType', v as TitleType)}
                    ...
                    {form.titleType === 'LEASEHOLD' && <InputField label="Leasehold Years Remaining" value={form.leaseholdYearsRemaining} onChange={v => setField('leaseholdYearsRemaining', v)} placeholder="e.g. 85" suffix="yrs" type="number" />}
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-6">
                    <InputField label="Year Built" value={form.yearBuilt} onChange={v => setField('yearBuilt', v)} placeholder="e.g. 2005" type="number" />
                    <InputField label="Last Refurb Year" value={form.yearRefurbished} onChange={v => setField('yearRefurbished', v)} placeholder="e.g. 2021" type="number" />
                    <InputField label="Total Built-Up Area" value={form.totalBuiltUpAreaSqm} onChange={v => setField('totalBuiltUpAreaSqm', v)} placeholder="GLA" suffix="SqM" type="number" />
                    <InputField label="Land Area" value={form.landAreaAcres} onChange={v => setField('landAreaAcres', v)} placeholder="0.00" suffix="Acres" type="number" />
                  </div>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-x-8 gap-y-6">
                    <InputField label="Net Lettable Area" value={form.totalLettableAreaSqm} onChange={v => setField('totalLettableAreaSqm', v)} placeholder="NLA" suffix="SqM" type="number" />
                    <InputField label="Number of Floors" value={form.numberOfFloors} onChange={v => setField('numberOfFloors', v)} placeholder="e.g. 12" type="number" />
                    <InputField label="Units / Suites" value={form.numberOfUnits} onChange={v => setField('numberOfUnits', v)} placeholder="e.g. 48" type="number" />
                    <InputField label="Parking Bays" value={form.numberOfParkingBays} onChange={v => setField('numberOfParkingBays', v)} placeholder="e.g. 200" type="number" />
                  </div>
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                      <SelectField label="Building Grade" value={form.buildingGrade} onChange={v => setField('buildingGrade', v as BuildingGrade)}
                      ...
                      <SelectField label="Green Certification" value={form.greenCertification} onChange={v => setField('greenCertification', v as GreenCert)}
                      ...
                      <InputField label="ESG Compliance Score" value={form.esgComplianceScore} onChange={v => setField('esgComplianceScore', v)} placeholder="0-100" type="number" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                      <InputField label="Last Independent Valuation" value={form.lastValuationAmount} onChange={v => setField('lastValuationAmount', v)} prefix="KES" placeholder="0" type="number" />
                      <InputField label="Valuation Date" value={form.lastValuationDate} onChange={v => setField('lastValuationDate', v)} type="date" />
                      <InputField label="Insurance Value" value={form.insuranceValue} onChange={v => setField('insuranceValue', v)} prefix="KES" placeholder="0" type="number" />
                      <InputField label="Property Manager" value={form.propertyManagerName} onChange={v => setField('propertyManagerName', v)} placeholder="e.g. Knight Frank Kenya" />
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {step === 4 && (
                <motion.div key="step4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                      <SelectField label="Primary Currency" value={form.currency} onChange={v => setField('currency', v as Currency)} options={[{ value: 'KES', label: 'KES - Kenyan Shilling' }, { value: 'USD', label: 'USD - US Dollar' }, { value: 'EUR', label: 'EUR - Euro' }, { value: 'GBP', label: 'GBP - British Pound' }]} />
                      <InputField label="Asking Price" value={form.askingPrice} onChange={v => setField('askingPrice', v)} required prefix={form.currency} placeholder="0" type="number" hint="Minimum KES 30M for land / KES 50M for income-generating" />
                      ...
                      {form.openToUsdPricing && <InputField label="USD Equivalent Asking Price" value={form.askingPriceUsd} onChange={v => setField('askingPriceUsd', v)} prefix="USD" placeholder="0" type="number" />}
                      <InputField label="Price per SqM (Auto-calculated)" value={form.askingPricePerSqm} onChange={v => setField('askingPricePerSqm', v)} prefix="KES" placeholder="Auto" type="number" hint="Based on asking price and built-up area" />
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                      <InputField label="Gross Rental Income (Annual)" value={form.grossRentAnnual} onChange={v => setField('grossRentAnnual', v)} prefix="KES" placeholder="0" type="number" hint="Total contracted gross rent per annum" />
                      <InputField label="Net Operating Income (NOI)" value={form.netOperatingIncome} onChange={v => setField('netOperatingIncome', v)} prefix="KES" placeholder="0" type="number" hint="Gross rent minus operating expenses and vacancy" />
                      <InputField label="Occupancy Rate" value={form.occupancyRate} onChange={v => setField('occupancyRate', v)} suffix="%" placeholder="e.g. 85" type="number" hint="Current percentage of space leased" />
                      <InputField label="WALE - Weighted Average Lease Expiry" value={form.waleYears} onChange={v => setField('waleYears', v)} suffix="Years" placeholder="e.g. 4.5" type="number" hint="Income-weighted average lease term remaining" />
                      <InputField label="Stabilised Cap Rate (Auto-calc)" value={form.capRate} onChange={v => setField('capRate', v)} suffix="%" placeholder="NOI / Price" type="number" hint="Auto-calculated. Override if needed." />
                      <InputField label="Annual Service Charge" value={form.serviceChargeAnnual} onChange={v => setField('serviceChargeAnnual', v)} prefix="KES" placeholder="0" type="number" hint="Total annual service charge" />
                      <InputField label="Service Charge per SqM" value={form.serviceChargePerSqm} onChange={v => setField('serviceChargePerSqm', v)} prefix="KES" placeholder="0" type="number" hint="Per square meter rate" />
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                      <InputField label="Existing Debt / Mortgage Balance" value={form.currentDebt} onChange={v => setField('currentDebt', v)} prefix="KES" placeholder="0 (if unencumbered)" type="number" hint="Outstanding loan balance" />
                      <InputField label="Debt Maturity / Facility Expiry" value={form.debtMaturityDate} onChange={v => setField('debtMaturityDate', v)} type="date" />
                      <InputField label="Debt Lender" value={form.debtLender} onChange={v => setField('debtLender', v)} placeholder="e.g. KCB, Stanbic, NCBA" />
                      <InputField label="Debt Interest Rate" value={form.debtInterestRate} onChange={v => setField('debtInterestRate', v)} placeholder="e.g. 13.5" suffix="%" type="number" />
                      <InputField label="LTV Ratio" value={form.ltvRatio} onChange={v => setField('ltvRatio', v)} placeholder="e.g. 45" suffix="%" type="number" hint="Loan-to-value ratio" />
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <SelectField label="Primary Reason for Divestment" value={form.divestmentReason} onChange={v => setField('divestmentReason', v as DivestmentReason)} options={DIVESTMENT_REASONS} required />
                    <SelectField label="Exclusivity Period Requested" value={form.exclusivityPeriod} onChange={v => setField('exclusivityPeriod', v as '30' | '60' | '90')}
                  ...
                  {form.divestmentReason === 'OTHER' && <InputField label="Describe Reason for Divestment" value={form.divestmentReasonOther} onChange={v => setField('divestmentReasonOther', v)} placeholder="Please provide details..." />}
                </motion.div>
              )}
              ...
              {step === 5 && (
                <motion.div key="step5" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                      <InputField label="Anchor Tenant Name" value={form.anchorTenantName} onChange={v => setField('anchorTenantName', v)} placeholder="e.g. Safaricom, Standard Chartered" />
                      <InputField label="Anchor Tenant Sector" value={form.anchorTenantSector} onChange={v => setField('anchorTenantSector', v)} placeholder="e.g. Banking, Telecom, Retail" />
                      <InputField label="Anchor Lease Expiry" value={form.anchorTenantLeaseExpiry} onChange={v => setField('anchorTenantLeaseExpiry', v)} type="date" />
                      <InputField label="Anchor Tenant Area" value={form.anchorTenantSqm} onChange={v => setField('anchorTenantSqm', v)} placeholder="0" suffix="SqM" type="number" />
                      <InputField label="Number of Tenants" value={form.numberOfTenants} onChange={v => setField('numberOfTenants', v)} placeholder="e.g. 8" type="number" />
                      <InputField label="Vacant Area" value={form.vacantArea} onChange={v => setField('vacantArea', v)} placeholder="0" suffix="SqM" type="number" />
                    </div>
                  </div>
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                      <InputField label="Total Lettable Area (NLA)" value={form.totalLettableArea} onChange={v => setField('totalLettableArea', v)} placeholder="0" suffix="SqM" type="number" hint="Net Lettable Area across all floors" />
                      <InputField label="Vacancy Rate" value={form.vacancyRate} onChange={v => setField('vacancyRate', v)} placeholder="0" suffix="%" type="number" />
                    </div>
                    <div className="space-y-2">
                  ...
                  <div className="p-6 bg-[#F8F7F4] border border-[#E5E2DC] space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <CheckboxField label="Water Recycling System" checked={form.waterRecycling} onChange={v => setField('waterRecycling', v)} />
                      <CheckboxField label="Solar Power Installation" checked={form.solarInstalled} onChange={v => setField('solarInstalled', v)} />
                      <CheckboxField label="Backup Generator" checked={form.backupGenerator} onChange={v => setField('backupGenerator', v)} />
                      <CheckboxField label="Borehole Water" checked={form.borehole} onChange={v => setField('borehole', v)} />
                      <CheckboxField label="Fiber Internet Connectivity" checked={form.fiberConnectivity} onChange={v => setField('fiberConnectivity', v)} />
                      <CheckboxField label="CCTV Security System" checked={form.cctvSecurity} onChange={v => setField('cctvSecurity', v)} />
                      <CheckboxField label="Access Control System" checked={form.accessControl} onChange={v => setField('accessControl', v)} />
                      <CheckboxField label="Fire Suppression System" checked={form.fireSuppression} onChange={v => setField('fireSuppression', v)} />
                    </div>
                    <InputField label="Energy Performance Rating" value={form.energyRating} onChange={v => setField('energyRating', v)} placeholder="e.g. A+, B, C" hint="If available from energy audit" />
                  </div>
                </motion.div>
              )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      {(form.role === 'DEVELOPER' || form.role === 'REIT' || form.role === 'FAMILY_OFFICE' || form.role === 'SOVEREIGN_FUND') && (
                        <>
                          <FileUploadZone category="ENTITY_REG" label="Certificate of Incorporation / Registration" required={true} accept=".pdf" desc="Company registration certificate" files={files} onAdd={addFiles} onRemove={removeFile} />
                          <FileUploadZone category="BOARD_RESOLUTION" label="Board Resolution to Sell / Divest" required={true} accept=".pdf" desc="Authorized by board of directors" files={files} onAdd={addFiles} onRemove={removeFile} />
                          <FileUploadZone category="AUDITED_ACCOUNTS" label="Audited Financial Statements (3 Years)" required={false} accept=".pdf,.xlsx" desc="Where applicable to the entity" files={files} onAdd={addFiles} onRemove={removeFile} />
                        </>
                      )}
                    </div>
                  </div>
                ...
                <motion.div key="step7" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
                  ...
                  <div className="bg-[#F8F7F4] border border-[#E5E2DC] p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { label: 'Role', value: ROLE_OPTIONS.find(r => r.value === form.role)?.label || form.role },
                        { label: 'Property', value: form.propertyName || 'Not specified' },
                        { label: 'LR Number', value: form.lrNumber || 'Not specified' },
                        { label: 'Asset Class', value: ASSET_CLASS_OPTIONS.find(a => a.value === form.assetClass)?.label || '' },
                        { label: 'Location', value: `${form.locationCity}${form.locationNeighborhood ? ', ' + form.locationNeighborhood : ''}` },
                        { label: 'Asking Price', value: form.askingPrice ? `${form.currency} ${Number(form.askingPrice).toLocaleString()}` : 'N/A' },
                        { label: 'Cap Rate', value: form.capRate ? `${form.capRate}%` : 'N/A' },
                        { label: 'Documents', value: `${files.length} file(s)` },
                        { label: 'Exclusivity', value: `${form.exclusivityPeriod} Days` },
                        { label: 'WALE', value: form.waleYears ? `${form.waleYears} yrs` : 'N/A' },
                      ].map(item => (
