...
/**
 * MURIVEST US PROPERTIES — INSTITUTIONAL PORTFOLIO VIEW
 * =====================================================
 * US commercial real estate (multifamily) portfolio interface.
 *
 * Design Language: Old Money / Golf Club Lounge (unchanged)
 * Typography: Editorial serif (Cormorant Garamond) + Swiss sans (Inter)
 * Palette: Forest Green, Warm Cream, Brass Gold, Charcoal
 *
 * Data source: Sanity _type == "usProperty" (CBRE Deal Flow research set)
 */
...
// ─── Score Helpers (activate automatically once coordinates are enriched) ────
      ...
      <div className="p-3 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <span className="text-xs text-[#2C2C2C]">{property.sqft || '—'}</span>
          </div>
        </div>

        {/* Walk Scores - Minimalist Row (renders once coordinates are enriched) */}
        {property.walkScores && (
        ...
        <>
          ...
          >
            ...
            <div className="relative h-[300px] overflow-hidden">
              {imgSrc ? (
                ...
                <div className="w-full h-full bg-[#1B4332] flex items-center justify-center">
                  <Building2 className="w-12 h-12 text-[#B8956B]" strokeWidth={1} />
                </div>
              )}
            ...
            <div className="p-6 space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#E8E6E1]">
                  <div>
                    ...
                    <p className="text-xl font-serif text-[#2C2C2C]">{property.price || 'Price on Request'}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-[9px] tracking-[0.2em] uppercase text-[#8B8680] block mb-1">
                      Size
                    ...
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <span className="text-sm font-medium text-[#2C2C2C]">{property.sqft || '—'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Walk Scores - Enhanced Section (renders once coordinates are enriched) */}
              {property.walkScores && (
                <div className="bg-white border border-[#E8E6E1] p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="text-center space-y-2">
                      <WalkScoreRing score={property.walkScores.walk} size={64} strokeWidth={4} showLabel={false} />
                      <div>
                    ...
                    {property.walkScores.transit !== null && (
                      <div className="text-center space-y-2">
                        <WalkScoreRing score={property.walkScores.transit} size={64} strokeWidth={4} showLabel={false} />
                        <div>
                    ...
                    {property.walkScores.bike !== null && (
                      <div className="text-center space-y-2">
                        <WalkScoreRing score={property.walkScores.bike} size={64} strokeWidth={4} showLabel={false} />
                        <div>
                    ...
                    {property.details
                      ...
                      .map((detail: any, i: number) => (
                        <div key={i} className="flex justify-between items-center py-2 border-b border-[#E8E6E1] last:border-0">
                          ...
                          <span className="text-sm text-[#2C2C2C] text-right">{detail.value}</span>
                        </div>
                      ))}
                  </div>
                </div>
              ...
              <div className="space-y-3 pt-4">
                <NextLink
                  href={`/usa/properties/${property.slug}`}
                  onClick={onClose}
...
const PropertyCard: React.FC<{
  ...
  onQuickView: (property: USAProperty) => void;
  index: number;
      ...
      <div className="relative h-[260px] overflow-hidden bg-[#F5F4F0]">
        ...
        {property.status && (
          <div className="absolute bottom-4 left-4">
            <span className="bg-white/95 backdrop-blur-sm text-[#2C2C2C] text-[9px] tracking-[0.15em] uppercase px-3 py-1.5 font-medium border border-[#E8E6E1]">
              {property.status}
            </span>
          </div>
        )}
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-[#E8E6E1]">
          <div>
            ...
            <p className="text-lg font-serif text-[#2C2C2C]">{property.price || 'Price on Request'}</p>
          </div>
          <div className="text-right">
            <span className="text-[9px] tracking-[0.2em] uppercase text-[#8B8680] block mb-0.5">
              Size
...
const PropertyStrip: React.FC<{
  properties: USAProperty[];
  hoveredId: string | null;
  onHover: (id: string | null) => void;
  onClick: (property: USAProperty) => void;
            ...
            return (
              ...
              >
                <div className="flex gap-4 p-4">
                  <div className="w-24 h-24 flex-shrink-0 overflow-hidden bg-[#1B4332] flex items-center justify-center">
                    {thumb ? (
                      <img src={thumb} alt={property.title} className="w-full h-full object-cover" loading="lazy" />
                    ) : (
                      <Building2 className="w-6 h-6 text-[#B8956B]" strokeWidth={1} />
                    )}
                  </div>
                  <div className="min-w-0 flex-1 space-y-1 py-1">
                    ...
                    <p className="text-sm font-serif text-[#2C2C2C] mt-2">{property.price || 'Price on Request'}</p>
                  </div>
                </div>
              </motion.div>
            );
...
// ─── Walk Score Fetcher (activates once coordinates are enriched) ────────────
...
interface PropertiesProps {
  initialData: USAProperty[];
}

export default function USAProperties({ initialData }: PropertiesProps) {
  const [properties, setProperties] = useState<USAProperty[]>(initialData || []);
  const [loading, setLoading] = useState(!initialData?.length);
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedType, setSelectedType] = useState('All');
  const [selectedListing, setSelectedListing] = useState('All');
  const [hoveredId, setHoveredId] = useState<string | null>(null);
  const [drawerProperty, setDrawerProperty] = useState<USAProperty | null>(null);
  ...
  useEffect(() => {
    if (!initialData?.length) {
      ...
      sanityClient
        .fetch(`*[_type == "usProperty" && !(_id in path("drafts.**"))] | order(_createdAt desc) {
          _id, title, "slug": slug.current, subtitle, location, city, state, zipCode,
          price, yield, sqft, assetType, type, status, listingType,
          "propertyType": coalesce(assetType, type, "residential"),
          description, features, details, investment, regulatory,
        ...
        .then(async (data) => {
          setProperties(data);
          setLoading(false);
          // Fetch walk scores for properties with coordinates (future enrichment)
          const enriched = await Promise.all(
            data.map(async (p: USAProperty) => {
      ...
      // Enrich initial data with walk scores (no-op until coordinates exist)
      Promise.all(
  ...
  const filteredProperties = useMemo(
    () =>
      properties.filter((p) => {
        ...
        const searchableLocation = [p.city, p.state, p.zipCode].filter(Boolean).join(' ').toLowerCase();
        const matchSearch =
          !q ||
          p.title?.toLowerCase().includes(q) ||
          p.city?.toLowerCase().includes(q) ||
          p.state?.toLowerCase().includes(q) ||
          searchableLocation.includes(q);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-xs tracking-[0.3em] uppercase text-[#8B8680]">Loading US Portfolio</p>
      </div>
    );
  }
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto">
          ...
          >
            ...
            <p className="text-[13px] text-white/70 font-light max-w-2xl mx-auto leading-relaxed tracking-wide">
              Institutional multifamily and residential investment assets across major US metro markets.
        ...
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 py-4">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
            ...
            <div className="relative w-full lg:w-80">
              ...
              <input
                type="text"
                placeholder="Search US assets by city, state or name..."
                value={searchTerm}
      ...
      <footer className="border-t border-[#E8E6E1] bg-white py-12">
        <div className="max-w-[1600px] mx-auto px-6 lg:px-12 text-center">
          ...
          <p className="text-xs text-[#8B8680] leading-relaxed max-w-2xl mx-auto font-light">
            All US properties are subject to availability and require qualified investor verification
            (KYC/AML). Full due diligence materials provided under NDA. Murivest Group Ltd is an
            independent advisory firm and does not operate collective investment schemes.
